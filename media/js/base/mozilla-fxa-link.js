...
(function() {
    ...
    FxaLink.interceptFxANavigation = function(event) {
        ...
        var extraURLParams = {};
        var allowedChars = /^[\w/.%-]+$/;
        var knownParams = ['flowId', 'flowBeginTime', 'deviceId',
        ...
        for (var i = 0; i < knownParams.length; i++) {
            ...
            // convert the param to snake_case for the UITour API
            var knownFormat = known.replace(/([A-Z])/g, ' $1');
            knownFormat = knownFormat.split(' ').join('_').toLowerCase();
            if (Object.prototype.hasOwnProperty.call(urlParams, known)) {
                ...
                if ((allowedChars).test(param)) {
                    extraURLParams[knownFormat] = param;
                }
            }
        }
